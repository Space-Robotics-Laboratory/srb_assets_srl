    ...
    class Exporter:
        ...
        def sdf_export(
            ...
            **kwargs,
            ...
            xml_str = xml_dom.parseString(xml_et.tostring(sdf, encoding="unicode"))
            xml_str = xml_str.toprettyxml(indent="  ")
            sdf_file = open(filepath_model.as_posix(), "w")
            sdf_file.write(xml_str)
            sdf_file.close()

            # Write the model config file
            cfg = xml_et.Element("model")
            name = xml_et.SubElement(cfg, "name")
            name.text = model_name
            version = xml_et.SubElement(cfg, "version")
            version.text = "1"
            sdf_tag = xml_et.SubElement(cfg, "sdf", attrib={"version": "1.9"})
            sdf_tag.text = "model.sdf"
            author = xml_et.SubElement(cfg, "author")
            producer = xml_et.SubElement(author, "producer")
            producer.text = f"Blender {bpy.app.version_string}"
            description = xml_et.SubElement(cfg, "description")
            description.text = "Procedurally generated model"
            xml_str = xml_dom.parseString(xml_et.tostring(cfg, encoding="unicode"))
            xml_str = xml_str.toprettyxml(indent="  ")
            model_cfg_file = open(filepath_config.as_posix(), "w")
            model_cfg_file.write(xml_str)
            model_cfg_file.close()

            # Render the thumbnail
            if render_thumbnail:
                obj_bbox_center = bpy.context.object.matrix_world @ (
                    sum(
                        (mathutils.Vector(b) for b in bpy.context.object.bound_box),
                        mathutils.Vector(),
                    )
                    / 8.0
                )
                obj_dims = bpy.context.object.dimensions
                ...
                bpy.context.scene.camera.data.ortho_scale = max_xy_dim
                bpy.context.scene.camera.data.clip_end = 2.0 * obj_dims[2]
                bpy.context.scene.render.resolution_x = math.ceil(
                    thumbnail_resolution * max_xy_dim / obj_dims[1]
                )
                bpy.context.scene.render.resolution_y = math.ceil(
                    thumbnail_resolution * max_xy_dim / obj_dims[0]
                )
                bpy.context.scene.render.filepath = filepath_thumbnail.as_posix()
        ...
        def stl_export(
            ...
            **kwargs,
            ...
            """
            Export via `bpy.ops.wm.stl_export()`
            """
            ...
            if not isinstance(filepath, Path):
                filepath = Path(filepath)
            filepath = filepath.with_suffix(".stl").absolute()
...
def parse_cli_args() -> argparse.Namespace:
    ...
    group.add_argument(
        "--ext",
        type=str,
        help="""The file extension of the exported models (ABC: '.abc',\
                                                           FBX: '.fbx',\
                                                           GLTF: '.glb|.gltf',\
                                                           OBJ: '.obj',\
                                                           PLY: '.ply',\
    ...
    group.add_argument(
        "--render_thumbnail",
        action="store_true",
        help="If specified, generate a thumbnail image for each exported model (only for SDF export)",
        default=False,
    )
    group.add_argument(
        "--thumbnail_resolution",
        type=int,
        help="Resolution of the generated thumbnail images",
        default=512,
    )
    ...
    group.add_argument(
        "--render_samples",
        type=int,
        help="Number of samples to use for rendering the PBR textures",
        default=4,
    )
